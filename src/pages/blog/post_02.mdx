...
Ahora vamos a resetear los estilos que por defecto pone el navegador a los elementos HTML. Te dejo es código css aquí abajo 👇:
...
Si ahora con este código intentas cambiar el tema a oscuro, lo hará 🤩, pero no con la transición 😢 ya que no estamos utilizando la **API View Transition**, así que ahora vamos a usarla.
...
- `document.startViewTransition(callback)`: Este método acepta una función de retorno (callback) que contiene los cambios que deben ocurrir durante la transición. En este caso, la función `toggleMode` se pasa como callback, por lo que los cambios de clase (y, por tanto, de tema) ocurren dentro de una transición visual gestionada por la API.
...
- `toggleBtn.addEventListener('click', callback)`: Este método añade un "escuchador" de eventos que se activa cuando el botón es presionado.
